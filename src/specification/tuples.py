...
def parse_compilers(tuplestring):
    ...
    print("compiler list = ",tuplestring)
...
def read_tuples(tuplefile, info):
    ...
    info['platforms'] = parse_platforms(lines[0])
    info['languages'] = parse_languages(lines[1])
    info['compilers'] = parse_compilers(lines[2])
    info['mutators'] = parse_mutators(lines[3])
    info['mutatees'] = parse_mutatees(lines[4])
    info['tests'] = parse_tests(lines[5])
    info['rungroups'] = parse_rungroups(lines[6])
...
#  info['exceptions'] = parse_exceptions(f.readline())
    ...
    info['objects'] = parse_object_files(lines[7])
    f.close()
